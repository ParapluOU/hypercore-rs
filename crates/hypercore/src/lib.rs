...
/// End-to-end browser persistence: a real `Hypercore` over the OPFS-backed
/// `LogStore`, persisted and reconstituted across a close+reopen in a Web Worker.
/// This is the local-first payoff proven in a real browser. wasm + `opfs` feature
/// only; run via `just wasm-test`.
