...
| `move-to.js`, `snapshots.js`, `streams.js` | [~] | `hypercore` seek/snapshot/stream — **`snapshots.js` headline ported** (ADR-0032): `Hypercore::snapshot()` returns a self-contained by-value `Snapshot<T,C>` (owns the present blocks' bytes + tree + signed head at capture time) whose `length`/`fork`/`head`/`get` are immune to the core's later append/truncate/re-append ("snapshot does not change when original gets modified"), `signed_length(&core)` ≡ upstream `snapshot.signedLength` via the content-blind shared-prefix LCA, `get` past the end is `None` (our L1 form of `SNAPSHOT_NOT_AVAILABLE`), and a captured block is independently authenticated against the snapshot head (host-safe form of "snapshots are consistent"); the empty/static-length cases ("snapshots wait for ready" minus the async-ready/persistence plumbing) ported too. We **diverge** from upstream's shared-storage copy-on-write (we copy bytes by value — same observable behaviour) and **defer**: `signedLength`-over-replication, implicit per-call snapshotting during a live download, `createReadStream` (`streams.js`), and the session/atom cases (sessions/networking, out of scope). `move-to.js`'s `truncate(1)` primitive is ported (ADR-0024); move-to / streams proper still todo |
